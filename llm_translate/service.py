...
class TranslationService:
    ...
    def parse_project(self, project_id: str) -> None:
        ...
        if project.input_format != "markdown":
            raise ValueError("V1.0 MVP currently supports Markdown input only")
        source = self.source_path(project)
        blocks = self.parser.parse(project.id, source.read_text(encoding="utf-8"))
